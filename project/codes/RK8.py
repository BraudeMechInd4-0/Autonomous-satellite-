...
# Define the Butcher table for DP8
Butcher_table_DP8 = {
...
# RK8 method implementation
def RK8(f,tspan, r0, h):
    t_start, t_end = tspan
    ...
    for i in range(n - 1):
        k = np.zeros((len(Butcher_table_DP8['c']), len(r0)))
        ti = t[i]
        ri = r[i]
        ...
        for j, cj in enumerate(Butcher_table_DP8['c']):
            if j == 0:
                y_temp = ri  # No coefficients for the first step
            else:
                y_temp = ri + h * sum(Butcher_table_DP8['a'][j][l] * k[l] for l in range(min(j, len(Butcher_table_DP8['a'][j]))))
            k[j] = f(ti + cj * h, y_temp)

        # Update the state vector using the k values and the b coefficients
        ri_new = ri + h * sum(Butcher_table_DP8['b'][j] * k[j] for j in range(len(k)))
        r.append(ri_new)
        
    results = np.column_stack((t, r))  # Combine time and state values
    return results
...
def main():
    ...
    results = RK8(tspan, y0, h, func)
    
    # Printing results in organized format
