...
def compare_algorithms(
    ...
    if algorithm_name =="apci":
        results = apci("apci_final_positions_results.csv")
        # Interpolate baseline positions
        interp_func = interp1d(base_time_points, base_positions, axis=0, kind='linear', fill_value="extrapolate")
...
if 'error' not in result:
    ...
    if A is not None and m is not None:
        ...
        plot_all_algorithms(r0, v0, tle_data, year, month, day, hour, minute, second, A, m, C_D, 'apci_final_positions_results.csv"')
