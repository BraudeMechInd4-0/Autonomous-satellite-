...
def compare_algorithms(r0, v0, algorithm_name, algorithm_module, func_name, tle_data, year, month, day, hour, minute, second, A, m, C_D=2.2, **kwargs):
    ...
    for i in range(num_segments):
        ...
        if algorithm_name == "APCI":
            ...
            results = getattr(algorithm_module, func_name)(lambda t, y: a_c_func(t, y, A, m, C_D), gauss_lobatto_tspan, y0, **kwargs)
...
for sat_data in tle_data:
    ...
    if 'error' not in result:
        ...
        # Get the satellite-specific parameters A (cross-sectional area), m (mass), and C_D (drag coefficient)
        A, m, C_D = get_satellite_params(sat_data[0])

        # Ensure the parameters were found
        if A is not None and m is not None:
            # Compare algorithms for this satellite
            step_size = 0.1
            tol = 1e-6
            h_min = 1e-6
            compare_algorithms(r0, v0, "RK4", RK4, "rk4", [sat_data], year, month, day, hour, minute, second, 3.9e-6, 260, C_D, h=step_size)
            compare_algorithms(r0, v0, "RK8", RK8, "RK8", [sat_data], year, month, day, hour, minute, second, A, m, C_D, h=step_size)
            compare_algorithms(r0, v0, "ODE45", ODE45, "ode45", [sat_data], year, month, day, hour, minute, second, A, m, C_D, tol=tol, hmax=step_size, hmin=h_min)
            compare_algorithms(r0, v0, "ODE78", ODE78, "ode78", [sat_data], year, month, day, hour, minute, second, A, m, C_D, h=step_size)
            compare_algorithms(r0, v0, "ODE113", ODE113, "ODE113", [sat_data], year, month, day, hour, minute, second, A, m, C_D, tol=tol, hmax=step_size, hmin=h_min)
        else:
            print(f"Satellite parameters not found for {sat_data[0]}.")
    else:
        # Handle the error for this specific satellite
        print(f"Error computing initial position and velocity for satellite {sat_data[0]}: {result['error']}")
        # Log or skip the problematic satellite


# Print comparison summary for all satellites and algorithms
def print_comparison_summary():
    print("\n*************** Average Position Difference for All Satellites ***************")
    print(f"{'Satellite':<15}{'Algorithm':<10}{'Avg Position Difference (km)':<30}{'Execution Time (s)':<20}")
    print("-" * 75)
    for result in comparison_results:
        print(f"{result['satellite']:<15}{result['algorithm']:<10}{result['avg_position_difference']:<30.6f}{result['execution_time']:<20.6f}")
