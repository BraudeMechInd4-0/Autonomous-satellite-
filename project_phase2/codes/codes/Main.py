...
import time
from read_csv_results import read_results  # Import the `apci` function from the module
...
def compare_algorithms(
    ...
    tle_data, year, month, day, hour, minute, second, **kwargs
):
    ...
    base_path = os.path.join("csvfiles", tle_data[0][0])  # e.g., 'csvfiles/QIANFAN-4'

    # Construct the file name based on algorithm and segment configuration
    file_name = f"Satellite_{algorithm_name}_final_positions_{num_segments}_{n_points}.csv"
    csv_file_path = os.path.join(base_path, file_name)

    try:
        # Read the CSV file
        results = read_results(csv_file_path)
...
def plot_all_algorithms(r0, v0, tle_data, year, month, day, hour, minute, second, A, m, C_D):
    ...
    for i in range(0,len( num_segments)):
        ...
        for algorithm_name in algorithms:
                time_points, position_differences = compare_algorithms(
                    r0, v0, algorithm_name, None, baseResults,
                    num_segments[i], n_points[i], None,
                    tle_data, year, month, day, hour, minute, second
                )
